  ...
  [re-com/h-box
   ...
   :children [
              ...
              [re-com/single-dropdown
               :choices (vec (sort-by #(clojure.string/upper-case (:label %1)) 
...
(defn- current-template[]
  (let [templates (re-frame/subscribe [:templates])
    ...
    (if-not (nil? @current-template)
      (if edit?
        (etemplate/display-template (zip/xml-zip (:template @current-template)) @model {:det (name @det)})
        (otemplate/display-template (zip/xml-zip (:template @current-template)) @model {:det (name @det)})))))
...
(defn- focus-selector[]
  ...
  (let [data-model (re-frame/subscribe [:data-model])]
    [re-com/single-dropdown 
     :choices (vec (concat [(merge {:id -1 :label "Focus On"})]
